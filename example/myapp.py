...
settings = dict(
    PREV_LABEL='<i class="left chevron icon"></i>',
...
async def index(request):
    ...
    sql = "select name from users limit {}, {}".format(offset, per_page)
    cur.execute(sql)
    users = cur.fetchall()
    cur.close()
    conn.close()
    pagination = Pagination(request, total=total, record_name="users")
    return jinja.render("index.html", request, users=users, pagination=pagination)
